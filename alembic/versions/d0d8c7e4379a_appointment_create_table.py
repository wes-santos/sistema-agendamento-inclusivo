...
down_revision: Union[str, Sequence[str], None] = '667634bd82ae'
...
def upgrade() -> None:
    ...
    sa.Column('status', sa.Enum('SCHEDULED', 'CONFIRMED', 'CANCELLED', 'DONE', name='appointment_status_enum'), nullable=False),
    sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    sa.ForeignKeyConstraint(['family_id'], ['users.id'], name=op.f('fk_appointments_family_id_users')),
    sa.ForeignKeyConstraint(['professional_id'], ['users.id'], name=op.f('fk_appointments_professional_id_users')),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_appointments'))
    )
    op.create_index(op.f('ix_appointments_family_id'), 'appointments', ['family_id'], unique=False)
    op.create_index('ix_appt_family_start', 'appointments', ['family_id', 'start_at'], unique=False)
    op.create_index('ix_appt_family_status_start', 'appointments', ['family_id', 'status', 'start_at'], unique=False)
